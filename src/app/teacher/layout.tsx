...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { name: "Dashboard", href: "/teacher/dashboard", icon: <Home size={24} /> },
    { name: "Students", href: "/teacher/students", icon: <Users size={24} /> },
    { name: "Lesson Planner", href: "/teacher/planner", icon: <BookOpen size={24} /> },
    { name: "Settings", href: "/teacher/settings", icon: <Settings size={24} /> },
  ];
    ...
    <div className="flex h-screen bg-[#0b1020] text-white overflow-hidden font-inter">
      ...
      <aside className="w-[80px] bg-[#121826] border-r border-white/10 flex flex-col items-center py-8 z-10 shadow-2xl">
        ...
        <Link href="/" className="mb-12 hover:-translate-y-1 transition-transform">
          <Logo className="w-10 h-10 shadow-[0_0_15px_rgba(79,141,255,0.4)]" />
        </Link>
            ...
            return (
              <Link key={item.name} href={item.href}
                className={`relative flex items-center justify-center w-full aspect-square rounded-xl transition-all ${
                  active 
                  ? "bg-[#4f8dff] text-black shadow-[0_0_10px_rgba(79,141,255,0.3)]" 
      ...
      <main className="flex-1 h-full overflow-y-auto relative">
        <div className="absolute inset-0 z-0 pointer-events-none opacity-20 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#4f8dff] via-[#0b1020] to-[#0b1020]" />
